    ...
    def _handle_ik_callback(self, cb_id, chat_id, message_id, callback_q, parts):
        ...
        def send(text, markup=None):
            ...
            if markup is not None:
                payload['reply_markup'] = markup
            try:
                self.listener_session.post(f"{base_url}/sendMessage", json=payload, timeout=5)
            except Exception:
                pass
        try:
            import room_access as _ra
        ...
        if action == 'sel':
            name, cnt, samples, is_paid = _ik.room_brief(arg)
            if cnt <= 0:
                answer('대상이 없습니다 (이미 처리/활동 재개).', alert=True)
                return
            answer()
            if arg == 'all':
                body = '⚠️ <b>전체 ' + str(cnt) + '명</b>을 강퇴할까요?'
            else:
                warn = ' ⚠️유료방' if is_paid else ''
                smp = (chr(10) + '· ' + ', '.join(samples) + (' …' if cnt > len(samples) else '')) if samples else ''
                body = '<b>' + name + '</b>' + warn + ' · 미접속 ' + str(cnt) + '명을 강퇴할까요?' + smp
            send(body, markup={'inline_keyboard': [[
                {'text': '✅ 강퇴 실행', 'callback_data': 'IK|go|' + arg},
                {'text': '취소', 'callback_data': 'IK|x'}]]})
            return
        ...
        if action == 'go':
            ...
            def _kick_worker():
                try:
                    done, total, name = _ik.kick_cached(arg)
                    if total == 0:
                        txt = 'ℹ️ 대상이 없습니다 (이미 처리/활동 재개).'
                    elif done < total:
                        txt = ('✅ [' + str(name) + '] ' + str(done) + '명 강퇴 완료'
                               + ' (' + str(total - done) + '명은 활동재개/이미나감)')
                    else:
                        txt = '✅ [' + str(name) + '] ' + str(done) + '명 강퇴 완료'
                    self.listener_session.post(f"{base_url}/editMessageText",
                        json={'chat_id': chat_id, 'message_id': message_id, 'text': txt, 'parse_mode': 'HTML'}, timeout=10)
                except Exception as e:
                    logging.error(f"[IK go] {e}")
